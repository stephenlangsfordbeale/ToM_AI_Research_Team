...
def _consume_cli_flag(flag: str) -> str | None:
    prefix = f"{flag}="
...
OUTPUT_FAMILY_INPUT = os.environ.get("TOM_MODAL_OUTPUT_FAMILY")

missing_envs: list[str] = []
if not TRAIN_SOURCE_INPUT:
    missing_envs.append("TOM_TRAIN_SOURCE")
if not INCUMBENT_FAMILY_INPUT:
    missing_envs.append("TOM_INCUMBENT_FAMILY")
if not OUTPUT_FAMILY_INPUT:
    missing_envs.append("TOM_MODAL_OUTPUT_FAMILY")

if missing_envs:
    raise SystemExit(
        "modal_runner.py requires explicit launch settings in env vars. Missing: "
        + ", ".join(missing_envs)
    )

assert TRAIN_SOURCE_INPUT is not None
assert INCUMBENT_FAMILY_INPUT is not None
assert OUTPUT_FAMILY_INPUT is not None
...
INCUMBENT_FAMILY = INCUMBENT_FAMILY_INPUT
REMOTE_OUTPUT_FAMILY = OUTPUT_FAMILY_INPUT

APP_NAME = os.environ.get("TOM_MODAL_APP_NAME", "tom-modal-runner-20260414")
...
REMOTE_ENV = {
    ...
    "TOM_MODAL_WAIT_FOR_RESULTS": "1" if WAIT_FOR_RESULTS else "0",
}
...
image = (
    ...
    .add_local_file(TRAIN_SOURCE, remote_path=str(REMOTE_PROJECT_ROOT / "train.py"))
    ...
    .add_local_file(PROJECT_ROOT / "scripts" / "modal_runner.py", remote_path=str(REMOTE_PROJECT_ROOT / "scripts" / "modal_runner.py"))
)
...
def run_modal_seed(
    ...
    train_source = REMOTE_PROJECT_ROOT / "train.py"
    env_source = REMOTE_PROJECT_ROOT / "env.py"
    eval_source = REMOTE_PROJECT_ROOT / "eval.py"
    runner_source = REMOTE_PROJECT_ROOT / "scripts" / "modal_runner.py"
    ...
    code_provenance = {
        ...
        "runner_script": {
            ...
            "sha1": _sha1_file(runner_source),
        },
        "project_files": {
            "train.py": {
                "path": str(train_source),
                "sha1": _sha1_file(train_source),
            },
            "env.py": {
                "path": str(env_source),
                "sha1": _sha1_file(env_source),
            },
            "eval.py": {
                "path": str(eval_source),
                "sha1": _sha1_file(eval_source),
            },
        },
        "incumbent_checkpoint": {
            "path": str(checkpoint_path),
            "sha1": _sha1_file(checkpoint_path),
        },
    }

    _write_json(code_provenance_json, code_provenance)
    ...
    status_payload = {
        ...
        "incumbent_family": INCUMBENT_FAMILY,
        "output_family": REMOTE_OUTPUT_FAMILY,
        "train_source_local": str(TRAIN_SOURCE),
        "remote_train_path": str(REMOTE_PROJECT_ROOT / "train.py"),
        "target_total_episodes": target_total_episodes,
        "initial_checkpoint": str(checkpoint_path),
        "initial_checkpoint_train_episodes": start_episodes,
        "resume_checkpoint": str(resume_checkpoint),
    ...
    summary = {
        ...
        "code_provenance_json": str(code_provenance_json),
