...
TOM_EXPERIMENTS = ("none", "belief_uncertainty_wait", "contextual_right_of_way_switch", "rebus")
...
class ToMPolicy(nn.Module):
    ...
    def _apply_experiment_bolt_on(
        ...
        if self.tom_experiment == "rebus":
            logits = logits.clone()
            normalized_entropy = self._normalized_belief_entropy(belief)
            prediction_error = self._rebus_prediction_error(obs)
    ...
    def step(self, obs: torch.Tensor, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        if self.tom_experiment == "rebus":
            rebus_prediction_error = self._rebus_prediction_error(obs)
...
def build_policy_runner(model: PolicyModel, device: torch.device):
    def _runner(obs: np.ndarray, state_np: Optional[np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        with torch.no_grad():
            ...
            for key in ("rebus_alpha", "rebus_prediction_error", "rebus_surprise"):
                if key in extra:
                    out[key] = np.array(float(extra[key].squeeze(0).item()), dtype=np.float32)
            return out
