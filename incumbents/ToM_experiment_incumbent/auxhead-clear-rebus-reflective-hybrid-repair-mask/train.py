...
class ToMPolicy(nn.Module):
    ...
    def uses_reflective_rebus(self) -> bool:
        return self.tom_experiment in {"rebus_reflective", "rebus_reflective_hybrid_repair_mask"}

    def uses_hybrid_repair_mask(self) -> bool:
        return self.tom_experiment == "rebus_reflective_hybrid_repair_mask"

    def uses_reflection_policy_mask(self) -> bool:
        return self.tom_experiment == "rebus_reflective"

    def _uses_rebus_precision(self) -> bool:
        return self.tom_experiment in {"rebus", "rebus_reflective", "rebus_reflective_hybrid_repair_mask"}
    ...
    def update_reflective_state(self, state: torch.Tensor, outcome_surprise: torch.Tensor) -> torch.Tensor:
        if not self.uses_reflective_rebus():
            return state
        ...
        if self.uses_hybrid_repair_mask():
            triggered = surprise >= self.tom_rebus_repair_surprise_threshold
    ...
    def update_reflective_state_numpy(self, state_np: Optional[np.ndarray], outcome_surprise: float) -> Optional[np.ndarray]:
        ...
        if self.uses_hybrid_repair_mask() and state_np.shape[-1] >= self.hidden_dim + 2:
            ...
            if surprise >= self.tom_rebus_repair_surprise_threshold:
    ...
    def _apply_experiment_bolt_on(
        ...
        if self.tom_experiment in {"rebus", "rebus_reflective", "rebus_reflective_hybrid_repair_mask"}:
            logits = logits.clone()
            normalized_entropy = self._normalized_belief_entropy(belief)
            prospective_error = (
            ...
            if self.uses_reflection_policy_mask():
                ...
                hybrid_repair_mask = (reflection_active & repair_window_active & tight_context).to(logits.dtype)
...
def rollout_episode(
    ...
    while not done:
        ...
        if isinstance(model, ToMPolicy) and model.uses_reflective_rebus():
            ...
            state = model.update_reflective_state(state, surprise_t)
...
def analyze_choice_context_outcomes(
    ...
    for scenario in scenarios:
        ...
        while not done:
            ...
            if isinstance(model, ToMPolicy) and model.uses_reflective_rebus():
                state = model.update_reflective_state_numpy(state, outcome_surprise)
