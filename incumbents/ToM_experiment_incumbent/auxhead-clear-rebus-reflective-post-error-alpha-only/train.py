...
TOM_EXPERIMENTS = (
    ...
    "rebus",
    "rebus_reflective",
    "rebus_reflective_post_error_alpha_only",
)
...
class ToMPolicy(nn.Module):
    ...
    def uses_reflective_rebus(self) -> bool:
        return self.tom_experiment in {"rebus_reflective", "rebus_reflective_post_error_alpha_only"}

    def uses_reflection_policy_mask(self) -> bool:
        return self.tom_experiment == "rebus_reflective"

    def _uses_rebus_precision(self) -> bool:
        return self.tom_experiment in {"rebus", "rebus_reflective", "rebus_reflective_post_error_alpha_only"}

    def _split_policy_state(self, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.uses_reflective_rebus() and state.shape[-1] == self.hidden_dim + 1:
            return state[:, : self.hidden_dim], state[:, self.hidden_dim]
        reflection_memory = torch.zeros(state.shape[0], dtype=state.dtype, device=state.device)
        return state[:, : self.hidden_dim], reflection_memory

    def _pack_policy_state(self, hidden_state: torch.Tensor, reflection_memory: torch.Tensor) -> torch.Tensor:
    ...
    def update_reflective_state(self, state: torch.Tensor, outcome_surprise: torch.Tensor) -> torch.Tensor:
        if not self.uses_reflective_rebus():
            return state
        hidden_state, reflection_memory = self._split_policy_state(state)
        ...
        return self._pack_policy_state(hidden_state, next_memory)
    ...
    def update_reflective_state_numpy(self, state_np: Optional[np.ndarray], outcome_surprise: float) -> Optional[np.ndarray]:
        ...
        state_np[:, -1] = np.clip(
            self.tom_rebus_reflection_decay * state_np[:, -1]
            + (1.0 - self.tom_rebus_reflection_decay) * surprise,
            0.0,
            1.0,
        )
    ...
    def _apply_experiment_bolt_on(
        ...
        if self.tom_experiment in {"rebus", "rebus_reflective", "rebus_reflective_post_error_alpha_only"}:
            logits = logits.clone()
            normalized_entropy = self._normalized_belief_entropy(belief)
            prospective_error = (
    ...
    def step(self, obs: torch.Tensor, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        new_state = self._pack_policy_state(new_hidden_state, reflection_memory)
        return logits, new_state, extra
