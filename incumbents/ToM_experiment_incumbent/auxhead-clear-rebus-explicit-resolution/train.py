...
_env = _load_repo_module("env")
ACTION_NAMES = _env.ACTION_NAMES
ASSERT = _env.ASSERT
PARTNER_TYPES = _env.PARTNER_TYPES
PROBE = _env.PROBE
PROCEED = _env.PROCEED
Scenario = _env.Scenario
WAIT = _env.WAIT
YIELD = _env.YIELD
ToMCoordinationEnv = _env.ToMCoordinationEnv
fixed_validation_scenarios = _env.fixed_validation_scenarios

_eval = _load_repo_module("eval")
EvalMetrics = _eval.EvalMetrics
evaluate_policy = _eval.evaluate_policy
...
TOM_EXPERIMENTS = (
    ...
    "rebus",
    "rebus_reflective",
    "rebus_reflective_hybrid_repair_mask",
    "rebus_reflective_hybrid_explicit_mask",
    "rebus_reflective_hybrid_explicit_resolution",
)
...
class ToMPolicy(nn.Module):
    def __init__(
        ...
        tom_rebus_alpha_base: float,
        tom_rebus_prediction_error_gain: float,
        tom_rebus_expected_collision_rate: float,
        tom_rebus_reflection_gain: float,
        tom_rebus_reflection_decay: float,
        tom_rebus_reflection_threshold: float,
        tom_rebus_outcome_surprise_scale: float,
        tom_rebus_repair_surprise_threshold: float,
        tom_rebus_repair_window_steps: float,
        tom_rebus_tight_partner_pressure_threshold: float,
        tom_rebus_resolution_clear_rate: float,
    ) -> None:
        super().__init__()
        self.hidden_dim = hidden_dim
        ...
        self.tom_rebus_alpha_base = tom_rebus_alpha_base
        self.tom_rebus_prediction_error_gain = tom_rebus_prediction_error_gain
        self.tom_rebus_expected_collision_rate = tom_rebus_expected_collision_rate
        self.tom_rebus_reflection_gain = tom_rebus_reflection_gain
        self.tom_rebus_reflection_decay = tom_rebus_reflection_decay
        self.tom_rebus_reflection_threshold = tom_rebus_reflection_threshold
        self.tom_rebus_outcome_surprise_scale = tom_rebus_outcome_surprise_scale
        self.tom_rebus_repair_surprise_threshold = tom_rebus_repair_surprise_threshold
        self.tom_rebus_repair_window_steps = tom_rebus_repair_window_steps
        self.tom_rebus_tight_partner_pressure_threshold = tom_rebus_tight_partner_pressure_threshold
        self.tom_rebus_resolution_clear_rate = tom_rebus_resolution_clear_rate

    def uses_reflective_rebus(self) -> bool:
        return self.tom_experiment in {
            "rebus_reflective",
            "rebus_reflective_hybrid_repair_mask",
            "rebus_reflective_hybrid_explicit_mask",
            "rebus_reflective_hybrid_explicit_resolution",
    ...
    def uses_hybrid_repair_mask(self) -> bool:
        return self.tom_experiment in {
            "rebus_reflective_hybrid_repair_mask",
            "rebus_reflective_hybrid_explicit_mask",
            "rebus_reflective_hybrid_explicit_resolution",
    ...
    def uses_explicit_repair_mask(self) -> bool:
        return self.tom_experiment in {
            "rebus_reflective_hybrid_explicit_mask",
            "rebus_reflective_hybrid_explicit_resolution",
        }

    def uses_resolution_signal(self) -> bool:
        return self.tom_experiment == "rebus_reflective_hybrid_explicit_resolution"

    def uses_reflection_policy_mask(self) -> bool:
        return self.tom_experiment == "rebus_reflective"

    def _uses_rebus_precision(self) -> bool:
        return self.tom_experiment in {
            "rebus",
            "rebus_reflective",
            "rebus_reflective_hybrid_repair_mask",
    ...
    def _split_policy_state(self, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        if self.uses_hybrid_repair_mask() and state.shape[-1] >= self.hidden_dim + 2:
            return state[:, : self.hidden_dim], state[:, self.hidden_dim], state[:, self.hidden_dim + 1]
        if self.uses_reflective_rebus() and state.shape[-1] >= self.hidden_dim + 1:
            hidden_state = state[:, : self.hidden_dim]
            reflection_memory = state[:, self.hidden_dim]
            repair_window = torch.zeros(state.shape[0], dtype=state.dtype, device=state.device)
            return hidden_state, reflection_memory, repair_window
        reflection_memory = torch.zeros(state.shape[0], dtype=state.dtype, device=state.device)
        repair_window = torch.zeros_like(reflection_memory)
        return state[:, : self.hidden_dim], reflection_memory, repair_window

    def _pack_policy_state(
        self,
        hidden_state: torch.Tensor,
        reflection_memory: torch.Tensor,
        repair_window: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        if not self.uses_reflective_rebus():
            return hidden_state
        if self.uses_hybrid_repair_mask():
            if repair_window is None:
                repair_window = torch.zeros_like(reflection_memory)
            return torch.cat([hidden_state, reflection_memory.unsqueeze(-1), repair_window.unsqueeze(-1)], dim=-1)
        return torch.cat([hidden_state, reflection_memory.unsqueeze(-1)], dim=-1)

    def update_reflective_state(
        self,
        state: torch.Tensor,
        outcome_surprise: torch.Tensor,
        repair_surprise: Optional[torch.Tensor] = None,
        resolution_signal: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        if not self.uses_reflective_rebus():
            return state
        hidden_state, reflection_memory, repair_window = self._split_policy_state(state)
        surprise = outcome_surprise.to(dtype=state.dtype, device=state.device).view_as(reflection_memory)
        surprise = (surprise * self.tom_rebus_outcome_surprise_scale).clamp(0.0, 1.0).detach()
        next_memory = (
            self.tom_rebus_reflection_decay * reflection_memory.detach()
            + (1.0 - self.tom_rebus_reflection_decay) * surprise
        ).clamp(0.0, 1.0)
        if self.uses_resolution_signal():
            resolution = (
                torch.zeros_like(reflection_memory)
                if resolution_signal is None
                else resolution_signal.to(dtype=state.dtype, device=state.device).view_as(reflection_memory)
            )
            resolution = resolution.clamp(0.0, 1.0).detach()
            repaired_context = (repair_window.detach() > 0.0) & (resolution > 0.0)
            repair_resolution_floor = torch.full_like(resolution, 0.75)
            effective_resolution = torch.where(
                repaired_context,
                torch.maximum(resolution, repair_resolution_floor),
                resolution,
            )
            clear_fraction = (self.tom_rebus_resolution_clear_rate * effective_resolution).clamp(0.0, 0.95)
            next_memory = (next_memory * (1.0 - clear_fraction)).clamp(0.0, 1.0)
        if self.uses_hybrid_repair_mask():
            repair_signal = surprise if repair_surprise is None else repair_surprise.to(
                dtype=state.dtype, device=state.device
            ).view_as(reflection_memory)
            repair_signal = repair_signal.clamp(0.0, 1.0).detach()
            triggered = repair_signal >= self.tom_rebus_repair_surprise_threshold
            triggered_window = torch.full_like(repair_window, float(self.tom_rebus_repair_window_steps))
            next_window = torch.where(triggered, triggered_window, (repair_window.detach() - 1.0).clamp_min(0.0))
        else:
            next_window = repair_window
        return self._pack_policy_state(hidden_state, next_memory, next_window)

    def update_reflective_state_numpy(
        self,
        state_np: Optional[np.ndarray],
        outcome_surprise: float,
        repair_surprise: Optional[float] = None,
        resolution_signal: Optional[float] = None,
    ) -> Optional[np.ndarray]:
        if not self.uses_reflective_rebus() or state_np is None or state_np.shape[-1] < self.hidden_dim + 1:
            return state_np
        state_np = state_np.copy()
        surprise = float(np.clip(outcome_surprise * self.tom_rebus_outcome_surprise_scale, 0.0, 1.0))
        reflection_idx = self.hidden_dim
        state_np[:, reflection_idx] = np.clip(
            self.tom_rebus_reflection_decay * state_np[:, reflection_idx]
            + (1.0 - self.tom_rebus_reflection_decay) * surprise,
            0.0,
            1.0,
        )
        if self.uses_resolution_signal():
            resolution = 0.0 if resolution_signal is None else float(np.clip(resolution_signal, 0.0, 1.0))
            if state_np.shape[-1] >= self.hidden_dim + 2 and float(np.max(state_np[:, self.hidden_dim + 1])) > 0.0:
                resolution = max(resolution, 0.75 if resolution > 0.0 else 0.0)
            clear_fraction = float(np.clip(self.tom_rebus_resolution_clear_rate * resolution, 0.0, 0.95))
            state_np[:, reflection_idx] = np.clip(state_np[:, reflection_idx] * (1.0 - clear_fraction), 0.0, 1.0)
        if self.uses_hybrid_repair_mask() and state_np.shape[-1] >= self.hidden_dim + 2:
            repair_idx = self.hidden_dim + 1
            repair_signal = surprise if repair_surprise is None else float(np.clip(repair_surprise, 0.0, 1.0))
            if repair_signal >= self.tom_rebus_repair_surprise_threshold:
                state_np[:, repair_idx] = float(self.tom_rebus_repair_window_steps)
            else:
                state_np[:, repair_idx] = np.clip(state_np[:, repair_idx] - 1.0, 0.0, None)
        return state_np

    def _normalized_belief_entropy(self, belief: torch.Tensor) -> torch.Tensor:
    ...
    def _rebus_prediction_error(self, obs: torch.Tensor) -> torch.Tensor:
        conflict_pressure = 1.0 - (torch.abs(obs[:, 2]) / max(self.conflict_dist_threshold, 1e-6)).clamp(0.0, 1.0)
        bottleneck_pressure = 1.0 - (torch.abs(obs[:, 3]) / max(self.bottleneck_dist_threshold, 1e-6)).clamp(0.0, 1.0)
        partner_pressing = ((obs[:, 4].clamp(0.0, 1.0) - 0.5) / 0.5).clamp(0.0, 1.0)
        collision_rate_proxy = conflict_pressure * bottleneck_pressure * (0.5 + 0.5 * partner_pressing)
        expected_collision_rate = torch.full_like(collision_rate_proxy, self.tom_rebus_expected_collision_rate)
        return torch.abs(collision_rate_proxy - expected_collision_rate)

    def _rebus_alpha(self, prediction_error: torch.Tensor) -> torch.Tensor:
        return self.tom_rebus_alpha_base + self.tom_rebus_prediction_error_gain * torch.sigmoid(prediction_error)
    ...
    def _apply_experiment_bolt_on(
        ...
        logits: torch.Tensor,
        belief: torch.Tensor,
        rebus_prediction_error: Optional[torch.Tensor] = None,
        rebus_prospective_error: Optional[torch.Tensor] = None,
        rebus_reflective_error: Optional[torch.Tensor] = None,
        rebus_repair_window: Optional[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        diagnostics: Dict[str, torch.Tensor] = {}
        ...
        if self.tom_experiment == "belief_uncertainty_wait":
            logits = logits.clone()
            normalized_entropy = self._normalized_belief_entropy(belief)
        ...
        if self.tom_experiment in {
            ...
            normalized_entropy = self._normalized_belief_entropy(belief)
            prospective_error = (
                self._rebus_prediction_error(obs) if rebus_prospective_error is None else rebus_prospective_error
            )
            reflective_error = (
                torch.zeros_like(prospective_error) if rebus_reflective_error is None else rebus_reflective_error
            )
            repair_window = torch.zeros_like(prospective_error) if rebus_repair_window is None else rebus_repair_window
            prediction_error = prospective_error if rebus_prediction_error is None else rebus_prediction_error
            ...
            evidence_released = obs[:, 8] >= self.tom_context_tag_threshold
            margin_narrow = obs[:, 7] >= self.tom_context_tag_threshold
            partner_pressing = obs[:, 4] >= self.tom_rebus_tight_partner_pressure_threshold
            negotiation_window = conflict_near | bottleneck_near
            tight_context = contested | (evidence_released & margin_narrow) | (
                evidence_released & partner_pressing & negotiation_window
            )
            explicit_tight_context = contested | (
                evidence_released & negotiation_window & (margin_narrow | partner_pressing)
            )
            if self.uses_reflective_rebus():
                reflection_active = reflective_error >= self.tom_rebus_reflection_threshold
                repair_window_active = repair_window > 0.0
            else:
                reflection_active = torch.zeros_like(contested)
                repair_window_active = torch.zeros_like(contested)
            if self.uses_reflection_policy_mask():
                mask = ((uncertain & contested) | reflection_active).to(logits.dtype)
                hybrid_repair_mask = torch.zeros_like(mask)
            elif self.uses_hybrid_repair_mask():
                repair_context = explicit_tight_context if self.uses_explicit_repair_mask() else tight_context
                hybrid_repair_mask = (reflection_active & repair_window_active & repair_context).to(logits.dtype)
                mask = (uncertain & contested).to(logits.dtype) + hybrid_repair_mask
                mask = mask.clamp(0.0, 1.0)
            else:
                mask = (uncertain & contested).to(logits.dtype)
                hybrid_repair_mask = torch.zeros_like(mask)
            surprise = torch.sigmoid(prediction_error).to(logits.dtype)
            gated_surprise = mask * surprise

            logits[:, ASSERT] = logits[:, ASSERT] - self.tom_experiment_strength * gated_surprise
            logits[:, PROCEED] = logits[:, PROCEED] - 0.70 * self.tom_experiment_strength * gated_surprise
            logits[:, WAIT] = logits[:, WAIT] + 0.40 * self.tom_experiment_strength * gated_surprise
            logits[:, PROBE] = logits[:, PROBE] + 0.70 * self.tom_experiment_strength * gated_surprise
            logits[:, YIELD] = logits[:, YIELD] + 0.18 * self.tom_experiment_strength * gated_surprise

            diagnostics["belief_entropy"] = normalized_entropy
            diagnostics["rebus_prediction_error"] = prediction_error
            diagnostics["rebus_prospective_error"] = prospective_error
            diagnostics["rebus_reflective_error"] = reflective_error
            diagnostics["rebus_repair_window"] = repair_window
            diagnostics["rebus_reflection_active_mask"] = reflection_active.to(logits.dtype)
            diagnostics["rebus_repair_window_active_mask"] = repair_window_active.to(logits.dtype)
            diagnostics["rebus_tight_context_mask"] = tight_context.to(logits.dtype)
            diagnostics["rebus_explicit_tight_context_mask"] = explicit_tight_context.to(logits.dtype)
            diagnostics["rebus_hybrid_repair_mask"] = hybrid_repair_mask.to(logits.dtype)
            diagnostics["rebus_surprise"] = surprise
            diagnostics["rebus_policy_mask"] = mask
            diagnostics["experiment_mask"] = mask
            return logits, diagnostics
    ...
    def init_state(self, batch_size: int = 1, device: torch.device | None = None) -> torch.Tensor:
        device = device or torch.device("cpu")
        if self.uses_hybrid_repair_mask():
            state_dim = self.hidden_dim + 2
        else:
            state_dim = self.hidden_dim + 1 if self.uses_reflective_rebus() else self.hidden_dim
        return torch.zeros(batch_size, state_dim, device=device)

    def step(self, obs: torch.Tensor, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]]:
        hidden_state, reflection_memory, repair_window = self._split_policy_state(state)
        new_hidden_state = self.gru(obs, hidden_state)
        belief_logits = self.belief_head(new_hidden_state)
        partner_action_logits = self.partner_action_head(new_hidden_state)
        policy_belief_logits = belief_logits
        rebus_diag: Dict[str, torch.Tensor] = {}
        rebus_prediction_error: Optional[torch.Tensor] = None
        rebus_prospective_error: Optional[torch.Tensor] = None
        rebus_reflective_error: Optional[torch.Tensor] = None
        rebus_repair_window: Optional[torch.Tensor] = None
        if self._uses_rebus_precision():
            rebus_prospective_error = self._rebus_prediction_error(obs)
            rebus_reflective_error = reflection_memory
            rebus_repair_window = repair_window
            if self.uses_reflective_rebus():
                rebus_prediction_error = rebus_prospective_error + self.tom_rebus_reflection_gain * rebus_reflective_error
            else:
                rebus_prediction_error = rebus_prospective_error
            rebus_alpha = self._rebus_alpha(rebus_prediction_error).unsqueeze(-1)
            policy_belief_logits = rebus_alpha * belief_logits
            partner_action_logits = rebus_alpha * partner_action_logits
            rebus_diag = {
                "rebus_alpha": rebus_alpha.squeeze(-1),
                "rebus_prediction_error": rebus_prediction_error,
                "rebus_prospective_error": rebus_prospective_error,
                "rebus_reflective_error": rebus_reflective_error,
                "rebus_repair_window": rebus_repair_window,
                "raw_belief_logits": belief_logits,
            }
        belief = F.softmax(policy_belief_logits, dim=-1)
        logits = self.policy_head(torch.cat([new_hidden_state, belief], dim=-1))
        ...
        logits, experiment_diag = self._apply_experiment_bolt_on(
            obs=obs,
            logits=logits,
            belief=belief,
            rebus_prediction_error=rebus_prediction_error,
        ...
        extra = {"belief_logits": policy_belief_logits, "belief": belief, "partner_action_logits": partner_action_logits}
        extra.update(rebus_diag)
        extra.update(prior_diag)
        extra.update(experiment_diag)
        new_state = self._pack_policy_state(new_hidden_state, reflection_memory, repair_window)
        return logits, new_state, extra
...
def reflective_outcome_surprise(
    obs_pre: np.ndarray,
    scenario: Scenario,
    action: int,
    reward: float,
    info: Dict[str, float],
    belief_ce: float = 0.0,
    partner_action_ce: float = 0.0,
) -> float:
    evidence_released = float(obs_pre[8]) >= 0.5
    missed_preferred = float(evidence_released and action not in scenario.preferred_after)
    reward_shock = np.clip(-float(reward), 0.0, 1.5) / 1.5
    belief_surprise = np.clip(float(belief_ce), 0.0, 3.0) / 3.0
    partner_surprise = np.clip(float(partner_action_ce), 0.0, 3.0) / 3.0
    raw_surprise = (
        1.00 * float(info.get("collision", 0.0) > 0)
        + 0.70 * float(info.get("deadlock", 0.0) > 0)
        + 0.40 * missed_preferred
        + 0.25 * float(reward_shock)
        + 0.20 * float(belief_surprise)
        + 0.15 * float(partner_surprise)
    )
    return float(np.clip(raw_surprise, 0.0, 1.0))


def explicit_decision_error_surprise(
    obs_pre: np.ndarray,
    scenario: Scenario,
    action: int,
    reward: float,
    ...
    evidence_released = float(obs_pre[8]) >= 0.5
    if not evidence_released:
        return 1.0 if float(info.get("collision", 0.0) > 0) else 0.0

    conflict_near = abs(float(obs_pre[2])) <= 0.35
    bottleneck_near = abs(float(obs_pre[3])) <= 0.35
    contested = conflict_near and bottleneck_near
    negotiation_window = conflict_near or bottleneck_near
    ...
    step_after = float(info.get("step", 0.0))
    late_pressure = step_after >= max(1.0, 0.60 * float(max_steps))

    cautious_actions = {WAIT, YIELD, PROBE}
    decisive_actions = {PROCEED, ASSERT}
    preferred_cautious = all(a in cautious_actions for a in scenario.preferred_after)
    preferred_decisive = all(a in decisive_actions for a in scenario.preferred_after)
    missed_preferred = action not in scenario.preferred_after
    tight_after_error = contested or (
        negotiation_window and (margin_narrow or partner_pressing or urgency_high or throughput_bias)
    )
    reward_shock = np.clip(-float(reward), 0.0, 1.5) / 1.5

    if float(info.get("collision", 0.0) > 0):
        return 1.0
    if float(info.get("deadlock", 0.0) > 0):
        return 0.90

    explicit_error = 0.0
    if preferred_cautious and action in decisive_actions and (tight_after_error or reward_shock >= 0.04):
        explicit_error = max(explicit_error, 0.85)
    if preferred_decisive and action in (WAIT, YIELD) and (urgency_high or throughput_bias or late_pressure):
        explicit_error = max(explicit_error, 0.75)
    if preferred_decisive and action == PROBE and late_pressure:
        explicit_error = max(explicit_error, 0.55)
    if missed_preferred and tight_after_error and reward_shock >= 0.04:
        explicit_error = max(explicit_error, 0.60)

    return float(np.clip(explicit_error, 0.0, 1.0))


def reflective_resolution_signal(
    obs_pre: np.ndarray,
    scenario: Scenario,
    action: int,
    reward: float,
    info: Dict[str, float],
    repair_surprise: float,
    max_steps: int,
) -> float:
    if float(info.get("collision", 0.0)) > 0 or float(info.get("deadlock", 0.0)) > 0:
        return 0.0
    if repair_surprise >= 0.40:
        return 0.0
    ...
    step_after = float(info.get("step", 0.0))
    late_pressure = step_after >= max(1.0, 0.60 * float(max_steps))
    preferred_action = action in scenario.preferred_after
    clean_reward = float(reward) >= -0.02

    signal = 0.0
    if preferred_action and clean_reward:
        signal = max(signal, 0.35)
        if negotiation_window and (margin_narrow or partner_pressing or urgency_high or throughput_bias):
            signal = max(signal, 0.45)
        if late_pressure:
            signal = max(signal, 0.50)
    if preferred_action and float(reward) > 0.05:
        signal = max(signal, 0.40)
    if not preferred_action and float(reward) > 0.08 and not (margin_narrow and partner_pressing):
...
def rollout_episode(
    ...
    while not done:
        ...
        if "belief_logits" in extra:
            ...
            belief_ce_value = float(belief_loss.detach().item())
        if "partner_action_logits" in extra:
            ...
            partner_action_loss = F.cross_entropy(extra["partner_action_logits"], partner_action_target)
            partner_action_aux_losses.append(partner_action_loss)
            partner_action_ce_value = float(partner_action_loss.detach().item())

        if isinstance(model, ToMPolicy) and model.uses_reflective_rebus():
            outcome_surprise = reflective_outcome_surprise(
                obs_pre=obs_pre,
                scenario=scenario,
                action=action,
                reward=reward,
                info=info,
                belief_ce=belief_ce_value,
                partner_action_ce=partner_action_ce_value,
            )
            repair_surprise = (
                explicit_decision_error_surprise(
                    obs_pre=obs_pre,
                    scenario=scenario,
                    action=action,
                    reward=reward,
                ...
                if model.uses_explicit_repair_mask()
                else outcome_surprise
            )
            resolution_signal = (
                reflective_resolution_signal(
                    obs_pre=obs_pre,
                    scenario=scenario,
                    action=int(action_t.item()),
                    reward=reward,
                    info=info,
                    repair_surprise=repair_surprise,
                    max_steps=env.max_steps,
                )
                if model.uses_resolution_signal()
                else 0.0
            )
            surprise_t = torch.tensor([outcome_surprise], dtype=torch.float32, device=device)
            repair_t = torch.tensor([repair_surprise], dtype=torch.float32, device=device)
            resolution_t = torch.tensor([resolution_signal], dtype=torch.float32, device=device)
            state = model.update_reflective_state(state, surprise_t, repair_t, resolution_t)
    ...
    def _runner(obs: np.ndarray, state_np: Optional[np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        with torch.no_grad():
            ...
            for key in (
                ...
                "late_yield_mask",
                "late_commit_mask",
                "rebus_reflection_active_mask",
                "rebus_repair_window_active_mask",
                "rebus_tight_context_mask",
                "rebus_explicit_tight_context_mask",
                "rebus_hybrid_repair_mask",
            ):
                if key in extra:
...
def analyze_choice_context_outcomes(
    ...
    for scenario in scenarios:
        ...
        rebus_alpha_trace: List[float] = []
        rebus_prediction_error_trace: List[float] = []
        rebus_prospective_error_trace: List[float] = []
        rebus_reflective_error_trace: List[float] = []
        rebus_repair_window_trace: List[float] = []
        rebus_surprise_trace: List[float] = []
        rebus_outcome_surprise_trace: List[float] = []
        rebus_explicit_error_trace: List[float] = []
        rebus_resolution_signal_trace: List[float] = []
        ...
        while not done:
            ...
            rebus_alpha_trace.append(float(out.get("rebus_alpha", 0.0)))
            rebus_prediction_error_trace.append(float(out.get("rebus_prediction_error", 0.0)))
            rebus_prospective_error_trace.append(float(out.get("rebus_prospective_error", 0.0)))
            rebus_reflective_error_trace.append(float(out.get("rebus_reflective_error", 0.0)))
            rebus_repair_window_trace.append(float(out.get("rebus_repair_window", 0.0)))
            rebus_surprise_trace.append(float(out.get("rebus_surprise", 0.0)))
            ...
            repair_surprise = (
                ...
                if isinstance(model, ToMPolicy) and model.uses_explicit_repair_mask()
                else outcome_surprise
            )
            resolution_signal = (
                reflective_resolution_signal(
                ...
                if isinstance(model, ToMPolicy) and model.uses_resolution_signal()
                else 0.0
            )
            rebus_outcome_surprise_trace.append(outcome_surprise)
            rebus_explicit_error_trace.append(repair_surprise)
            rebus_resolution_signal_trace.append(resolution_signal)
            if isinstance(model, ToMPolicy) and model.uses_reflective_rebus():
                state = model.update_reflective_state_numpy(
                    state,
                    outcome_surprise,
                    repair_surprise,
                    resolution_signal,
        ...
        scenario_summaries.append(
            {
                ...
                "rebus_alpha_trace": rebus_alpha_trace,
                "rebus_prediction_error_trace": rebus_prediction_error_trace,
                "rebus_prospective_error_trace": rebus_prospective_error_trace,
                "rebus_reflective_error_trace": rebus_reflective_error_trace,
                "rebus_repair_window_trace": rebus_repair_window_trace,
                "rebus_surprise_trace": rebus_surprise_trace,
                "rebus_outcome_surprise_trace": rebus_outcome_surprise_trace,
                "rebus_explicit_error_trace": rebus_explicit_error_trace,
                "rebus_resolution_signal_trace": rebus_resolution_signal_trace,
...
def train(args: argparse.Namespace) -> EvalMetrics:
    ...
    if args.variant == "baseline":
        ...
        model = ToMPolicy(
            ...
            tom_rebus_alpha_base=args.tom_rebus_alpha_base,
            tom_rebus_prediction_error_gain=args.tom_rebus_prediction_error_gain,
            tom_rebus_expected_collision_rate=args.tom_rebus_expected_collision_rate,
            tom_rebus_reflection_gain=args.tom_rebus_reflection_gain,
            tom_rebus_reflection_decay=args.tom_rebus_reflection_decay,
            tom_rebus_reflection_threshold=args.tom_rebus_reflection_threshold,
            tom_rebus_outcome_surprise_scale=args.tom_rebus_outcome_surprise_scale,
            tom_rebus_repair_surprise_threshold=args.tom_rebus_repair_surprise_threshold,
            tom_rebus_repair_window_steps=args.tom_rebus_repair_window_steps,
            tom_rebus_tight_partner_pressure_threshold=args.tom_rebus_tight_partner_pressure_threshold,
            tom_rebus_resolution_clear_rate=args.tom_rebus_resolution_clear_rate,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--tom-rebus-alpha-base",
        type=float,
        default=0.70,
        help="Base belief-logit precision multiplier for the REBUS experiment.",
    )
    parser.add_argument(
        "--tom-rebus-prediction-error-gain",
        type=float,
        default=0.60,
        help="Gain applied to sigmoid(prediction_error) for REBUS dynamic precision.",
    )
    parser.add_argument(
        "--tom-rebus-expected-collision-rate",
        type=float,
        default=0.10,
        help="Expected collision-risk baseline used by the REBUS observable prediction-error proxy.",
    )
    parser.add_argument(
        "--tom-rebus-reflection-gain",
        type=float,
        default=0.75,
        help="Gain from retrospective outcome-surprise memory into reflective REBUS prediction error.",
    )
    parser.add_argument(
        "--tom-rebus-reflection-decay",
        type=float,
        default=0.85,
        help="Step-to-step decay for reflective REBUS outcome-surprise memory.",
    )
    parser.add_argument(
        "--tom-rebus-reflection-threshold",
        type=float,
        default=0.12,
        help="Reflective memory threshold that can independently activate the REBUS policy mask.",
    )
    parser.add_argument(
        "--tom-rebus-outcome-surprise-scale",
        type=float,
        default=1.0,
        help="Multiplier applied to collision/deadlock/regret/reward/prediction-loss surprise before memory update.",
    )
    parser.add_argument(
        "--tom-rebus-repair-surprise-threshold",
        type=float,
        default=0.40,
        help="Outcome-surprise threshold that opens a hybrid REBUS post-error repair window.",
    )
    parser.add_argument(
        "--tom-rebus-repair-window-steps",
        type=float,
        default=2.0,
        help="Number of subsequent steps a hybrid REBUS post-error repair window remains open.",
    )
    parser.add_argument(
        "--tom-rebus-tight-partner-pressure-threshold",
        type=float,
        default=0.72,
        help="Partner-pressure observation threshold used as part of the hybrid REBUS tight-context gate.",
    )
    parser.add_argument(
        "--tom-rebus-resolution-clear-rate",
        type=float,
        default=0.65,
        help="Maximum fraction of reflective REBUS memory cleared by clean completion or post-error resolution.",
    )
