...
class ToMPolicy(nn.Module):
    ...
    def step(self, obs: torch.Tensor, state: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        extra = {"belief_logits": belief_logits, "belief": belief}
...
def rollout_episode(
    ...
    deterministic: bool,
    scenario: Scenario,
) -> Tuple[List[torch.Tensor], List[float], List[torch.Tensor], List[torch.Tensor], List[torch.Tensor]]:
    ...
    return log_probs, rewards, entropies, belief_aux_losses, behavior_aux_losses
...
def train(args: argparse.Namespace) -> EvalMetrics:
    ...
    for episode in range(1, args.train_episodes + 1):
        ...
        log_probs, rewards, entropies, belief_aux_losses, behavior_aux_losses = rollout_episode(
            env,
            model,
            device,
            deterministic=False,
